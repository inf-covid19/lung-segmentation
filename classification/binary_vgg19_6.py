...
def get_data(train_img_src_folder, train_img_folders, validation_img_src_folder, validation_img_folders):   
    ...
    for (base, folder, dic) in df_config:
        for img_folder, img_label in folder.items():
            if img_folder[:3] == 'TYP':
            	search_folder = "{}/{}".format(base + '/Typical', img_folder[4:])
            elif img_folder[:3] == 'NEG':
            	search_folder = "{}/{}".format(base + '/Negative', img_folder[4:]) 
            else:
...
print('')
predictions_by_patient(model, validation_covid_folders, typical_src)
print('')
print('Predicting HCPA Negative Patients')
print('')
predictions_by_patient(model, validation_other_folders, negative_src)
