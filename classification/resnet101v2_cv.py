...
#than 2 classes.
        ...
        for i,test_df in enumerate(results):
                ...
                if os.path.isfile('saved_legends/resnet101v2.npy'):
                        print('loading label legend file')
                        class_indices = np.load('saved_legends/resnet101v2.npy', allow_pickle=True).item()
...
def train_model(model, train_df, validation_df, epochs, callbacks=[]):
	...
	np.save('saved_legends/resnet101v2.npy', train_generator.class_indices)
...
for i in range(5):
	...
	checkpoint_filepath = 'Resnet101V2/weights_' + str(i+1)
	...
	model.save('Resnet101V2/model_' + str(i+1))
