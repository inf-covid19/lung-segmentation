# -*- coding: utf-8 -*-
'''
binary classifier implemented with transfer learning, using vgg19
training set:

4/5 of patients described below, randomly selected:

	73 HMV typical classification, positive pcr patients
	29 HMV negative classification, negative pcr patients
	30 HCPA typical classification patients
	15 HCPA negative classification patients
	
	HMV indetermined classification patients
	
validation:

the other 1/5 of described patients.
...
#than 2 classes.
	...
	for i,test_df in enumerate(results):
		...
		if os.path.isfile('saved_legends/ternary_legend.npy'):
			print('loading label legend file')
			class_indices = np.load('saved_legends/ternary_legend.npy', allow_pickle=True).item()
...
def train_model(model, train_df, validation_df, epochs, callbacks=[]):
	...
	np.save('saved_legends/ternary_legend.npy', train_generator.class_indices)
...
def get_dicts(train_folders_1, train_folders_2, train_folders_3, validation_folders_1, validation_folders_2, validation_folders_3):
	...
	train_set.update(dict(zip(train_folders_3, ['indetermined' for i in range(len(train_folders_3))])))
	...
	validation_set.update(dict(zip(validation_folders_3, ['indetermined' for i in range(len(validation_folders_3))])))
...
train_negative_folders = []
train_typical_folders = []
train_indetermined_folders = []
...
for i in range(len(annotations)):
	if str(annotations["nome"][i]) != "nan":
		...
		p_id = 'C' + str(p_num)
		if classification == "2 - típico" and pcr == 1 and p_num != 53 and p_num != 95 and p_num != 153:
			train_typical_folders.append(p_id)
		elif classification == "1 - negativo" and pcr == 2:
			train_negative_folders.append(p_id)
		elif classification == "3 - indeterminado" and p_num != 1:
			train_indetermined_folders.append(p_id)
...
#adding HCPA patients to training set
for patient in os.listdir('HCPA-Processadas'+ '/' + 'Negative'):
	patient_name = 'NEG-' + patient
	train_negative_folders.append(patient_name)
	
for patient in os.listdir('HCPA-Processadas' + '/' + 'Typical'):
	patient_name = 'TYP-' + patient
	train_typical_folders.append(patient_name)
	
print('')
print("Found", len(train_negative_folders), "Negative patients")
print("Found", len(train_typical_folders), "Typical patients")
print("Found", len(train_indetermined_folders), "Indetermined patients")
print('')

#balancing classes
classes = []
classes.append(train_typical_folders)
classes.append(train_negative_folders)
classes.append(train_indetermined_folders)

classes, leftover = balance_classes(classes)
train_typical_folders = classes[0]
train_negative_folders = classes[1]
train_indetermined_folders = classes[2]

#splitting into train and validation
train_typical_folders, validation_typical_folders = train_validation_split(train_typical_folders)
train_negative_folders, validation_negative_folders = train_validation_split(train_negative_folders)
train_indetermined_folders, validation_indetermined_folders = train_validation_split(train_indetermined_folders)

#adding leftover patients from balancing to the validation sets
validation_typical_folders = validation_typical_folders + leftover[0]
validation_negative_folders = validation_negative_folders + leftover[1]
validation_indetermined_folders = validation_indetermined_folders + leftover[2]
...
print('Indetermined Patients in training set: ', len(train_indetermined_folders), train_indetermined_folders)
print('')
...
print('Indetermined Patients in validation set: ', len(validation_indetermined_folders), validation_indetermined_folders)
print('')

model = get_model()

#fitting a model
train_set, validation_set = get_dicts(train_negative_folders, train_typical_folders, train_indetermined_folders, 
		                              validation_negative_folders, validation_typical_folders, validation_indetermined_folders)

train_df, validation_df = get_data(train_set, validation_set)

#training the model and saving the label legends
checkpoint_filepath = 'saved_weights/ternary_weights/ternary_weights'
...
print('')
print('Predicting Indetermined Patients')
predictions_by_patient(model, validation_indetermined_folders)
print('')
