...
def predictions_by_patient(model, patients):
	...
	for i,test_df in enumerate(results):
		...
		if os.path.isfile('saved_legends/binary_cv_2.npy'):
			print('loading label legend file')
			class_indices = np.load('saved_legends/binary_cv_2.npy', allow_pickle=True).item()
...
def train_model(model, train_df, validation_df, epochs, callbacks=[]):
	...
	np.save('saved_legends/binary_cv_2.npy', train_generator.class_indices)
...
validation_negative_fold3 = ['C76', 'C159', 'C29', 'NEG-010', 'C1', 'C102', 'C50', 'NEG-014', 'C46']
...
for i in range(4,5):
	...
	checkpoint_filepath = 'BinaryCV_2/weights_' + str(i+1)
	...
	model.save('BinaryCV_2/model_' + str(i+1))
