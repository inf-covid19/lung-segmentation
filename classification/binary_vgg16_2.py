...
#than 2 classes.
	...
	for p in patients:
		...
		if len(os.listdir(curr_dir)) > 0: #checks if the folder is empty
	...
	for i,test_df in enumerate(results):
		...
		if os.path.isfile('vgg_legend_2.npy'):
			print('loading label legend file')
			class_indices = np.load('vgg_legend_2.npy', allow_pickle=True).item()
...
def train_model(model, train_df, validation_df, epochs, callbacks=[]):
	...
	np.save('vgg_legend_2', train_generator.class_indices)
...
#fitting a model
validation_other_folders = ['C'+str(p) for p in other_p[:6]]
validation_covid_folders = ['C'+str(p) for p in covid_p[:6]]
...
train_covid_folders = ['C' + str(p) for p in covid_p[6:30]]
#train_indefinite_folders = list(set(indefinite_patients) - set(validation_indefinite_folders))
test_covid_folders = list(set(covid_patients) - set(train_covid_folders))
...
train_model(model, train_df, validation_df, 15)
...
predictions_by_patient(model, test_covid_folders)
