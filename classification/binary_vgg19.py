...
#than 2 classes.
	...
	for i,test_df in enumerate(results):
		...
		if os.path.isfile('saved_legends/binary_legend.npy'):
			print('loading label legend file')
			class_indices = np.load('saved_legends/binary_legend.npy', allow_pickle=True).item()
...
def train_model(model, train_df, validation_df, epochs, callbacks=[]):
	...
	np.save('saved_legends/binary_balanced_legend.npy', train_generator.class_indices)
...
checkpoint_filepath = 'saved_weights/balanced_weights/balanced_weights'
